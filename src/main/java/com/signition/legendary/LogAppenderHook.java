...
public class LogAppenderHook {
    ...
    public void register() {
        try {
            ...
            String p1 = cfg.getString("match.legendary_line_regex",
                    "^\\[Pixelmon\\] (?<name>[A-Za-z'\\- ]+) has spawned in a (?P<biome>.+?) biome!$");
            String p2 = cfg.getString("match.coords_line_regex",
                    "^Spawned (?<name>[A-Za-z'\\- ]+) at: .* x:(?<x>-?\\d+), y:(?<y>-?\\d+), z:(?<z>-?\\d+)$");
            legendaryPattern = new RegexSafe(plugin.getLogger()).compileOrDefault(p1, "^\\[Pixelmon\\] (?<name>[A-Za-z'\\- ]+) has spawned in a (?<biome>.+?) biome!$", "match.legendary_line_regex");
            coordsPattern = new RegexSafe(plugin.getLogger()).compileOrDefault(p2, "^Spawned (?<name>[A-Za-z'\\- ]+) at:.* x:\\s*(?<x>-?\\d+),\\s*y:\\s*(?<y>-?\\d+),\\s*z:\\s*(?<z>-?\\d+)", "match.coords_line_regex");
